...
def load_data_variables_for_model(
    ...
    observed_hosp_admissions = filtered_data.select(
        ["location", "date", "hosp"]
    )
    # output dictionary
    out = {
...
def run_jurisdiction(
    ...
    data_obs_hosp = (
        variables_from_data["observations"]
        .select(pl.col("hosp"))
        .to_numpy()
        .flatten()
    )[:steps_excluding_forecast]
    ...
    return idata, variables_from_data["observations"]
...
def main(args):
    ...
    if args.historical_data:
        ...
        for jurisdiction in args.regions:
            ...
            idata, observed_hosp_admissions = run_jurisdiction(
            ...
            obs_data_by_jurisdiction[jurisdiction] = observed_hosp_admissions
        # create df of observations and save
        concatenated_obs = pl.concat(
            obs_data_by_jurisdiction.values(), how="vertical"
        )
        concatenated_obs.write_csv(obs_data_path)
